...
class MainViewModel : ViewModel() {
    ...
    var status = MutableStateFlow(LaundryApi.ApiStatus.LOADING)
        private set
    ...
    fun retrieveData(userId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            status.value = LaundryApi.ApiStatus.LOADING
            try {
                data.value = LaundryApi.service.getLaundry(userId)
                status.value = LaundryApi.ApiStatus.SUCCESS
            } catch (e: Exception) {
                Log.d("MainViewModel", "Failure: ${e.message}")
                status.value = LaundryApi.ApiStatus.FAILED
            }
        }
    }

    fun saveData(userId: String, nama: String,berat: String, jenis: String, bitmap: Bitmap) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
    ...
    fun deleteData(userId: String, Id: Int){
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val response = LaundryApi.service.deleteLaundry(userId, Id)
